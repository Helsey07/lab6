...
instances = [
    {'ip': 'http://127.0.0.1', 'port': 5001, 'status': 'active'},
    {'ip': 'http://127.0.0.1', 'port': 5002, 'status': 'active'},
    {'ip': 'http://127.0.0.1', 'port': 5003, 'status': 'active'}
]
...
def check_health():
    ...
    while True:
        for instance in instances:
            try:
                response = requests.get(f"{instance['ip']}:{instance['port']}/health", timeout=3)
...
def process():
    ...
    for _ in range(len(instances)):
        ...
        if instance['status'] == 'active':
            response = requests.get(f"{instance['ip']}:{instance['port']}/process")
...
@app.route('/')
def home():
    # Отправляем список инстансов в шаблон
